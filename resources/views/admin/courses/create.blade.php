        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
            <div class="flex items-center mb-6">
                <a href="{{ route('admin.courses.index') }}"
                    class="flex items-center text-gray-600 hover:text-purple-600 mr-4">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">
                            <i class="fas fa-{{ $field['icon'] }} text-purple-600 mr-2"></i>
                            {{ $field['label'] }}
                        </label>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                        ...
                        <div>
                            ...
                            <input type="time" name="exam_time" value="{{ old('exam_time') }}"
                                class="w-full px-4 py-3 border border-gray-300 rounded-lg">
                            @error('exam_time')
                            <span class="text-red-500 text-sm mt-1 block">{{ $message }}</span>
                            @enderror
                    ...
                    <div>
                        ...
                        <select name="course_exam_id"
                            ...
                            <option value="{{ $exam->id }}" {{ old('course_exam_id')==$exam->id ? 'selected' : '' }}>{{
                                $exam->title }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="4"
                            class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent">{{ old('description') }}</textarea>
                        @error('description')
                        <span class="text-red-500 text-sm mt-1 block">{{ $message }}</span>
                        @enderror
                    </div>
                                ...
                                <div class="md:col-span-12">
                                    <div class="space-y-2">
                                        <div>
                                            ...
                                            <input type="text" name="episodes[0][title]"
                                                value="{{ old('episodes.0.title') }}"
                                                class="w-full px-3 py-2 border rounded">
                                            @error('episodes.0.title')
                                            <span class="text-red-500 text-sm mt-1 block">{{ $message }}</span>
                                            @enderror
                                        </div>
                                        <div>
                                            <label class="text-sm text-gray-600">الرابط</label>
                                            <input type="url" name="episodes[0][url]"
                                                value="{{ old('episodes.0.url') }}"
                                                class="w-full px-3 py-2 border rounded">
                                            @error('episodes.0.url')
                                            <span class="text-red-500 text-sm mt-1 block">{{ $message }}</span>
                                            @enderror
                                        </div>
                                    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
                    ...
                    <div class="md:col-span-2">
                        <button type="button" 
                            ...
                            <i class="fas fa-times mr-2"></i> حذف
                        </button>
                    </div>
                </div>
            `;
