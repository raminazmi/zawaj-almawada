...
<section class="min-h-screen">
    <div class="max-w-7xl mx-auto px-4 py-8">
        <div class="flex justify-between items-center mb-8">
            <div>
                <h2 class="text-2xl font-bold text-purple-800">
                    <i class="fas fa-clock text-purple-600 ml-2"></i>الطلبات المنتهية
                </h2>
                <p class="text-sm text-gray-600 mt-2">
                    هذه الطلبات التي تمت الموافقة عليها وفي حالة الخطوبة ومرت عليها مدة شهرين من تاريخ إنشائها.
                </p>
            </div>

            @if($expiredRequests->isNotEmpty())
            <form method="POST" action="{{ route('admin.marriage-requests.delete-all-expired') }}"
                class="delete-all-expired">
                @csrf
                @method('DELETE')
                <button type="submit"
        ...
        <div class="bg-white rounded-2xl shadow-xl border border-purple-100 overflow-hidden">
            @forelse($expiredRequests as $request)
            <div class="p-6 border-b border-purple-50 group hover:bg-purple-50 transition-all">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                    ...
                    <div class="flex items-center gap-4 justify-end">
                        <form method="POST" action="{{ route('admin.marriage-requests.delete-expired', $request->id) }}"
            ...
            <div class="text-center p-12">
                ...
                <h3 class="text-xl font-semibold text-gray-800 mb-2">
                    لا توجد طلبات منتهية حالياً
                </h3>
            </div>
            @endforelse
        </div>

        @if($expiredRequests->hasPages())
        <div class="mt-6 px-4">
            {{ $expiredRequests->links('pagination::tailwind') }}
        </div>
        @endif
    </div>
</section>
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        const deleteForms = document.querySelectorAll('.delete-expired');
        ...
        const deleteAllForm = document.querySelector('.delete-all-expired');
        ...
        if (deleteAllForm) {
            deleteAllForm.addEventListener('submit', function (e) {
                ...
                Swal.fire({
                    title: 'هل أنت متأكد؟',
                    text: "سيتم حذف جميع الطلبات المنتهية نهائيًا!",
                    icon: 'warning',
                    showCancelButton: true,
                    confirmButtonColor: '#ef4444',
                    cancelButtonColor: '#6b7280',
