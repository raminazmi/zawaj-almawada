        ...
        <form action="{{ route('admin.questions.update', $question->id) }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <textarea id="male_question" name="male_question" rows="3" required
                    class="w-full mt-1 p-3 border border-gray-300 rounded-lg focus:ring-purple-500 focus:border-purple-500">{{ $question->male_question }}</textarea>
            </div>
            ...
            <div>
                ...
                <textarea id="female_question" name="female_question" rows="3" required
                    class="w-full mt-1 p-3 border border-gray-300 rounded-lg focus:ring-pink-500 focus:border-pink-500">{{ $question->female_question }}</textarea>
            </div>
                ...
                <div class="bg-gray-100 p-4 rounded-lg space-y-2">
                    ...
                    <div class="flex items-center gap-2">
                        <input type="checkbox" id="answer_{{ $loop->index }}" name="correct_answers[]"
                            value="{{ $answer }}" @checked(!in_array($answer, $question->wrong_answers))
