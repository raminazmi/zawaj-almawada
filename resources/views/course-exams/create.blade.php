                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <input type="text" name="title"
                            class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:ring-[#3A8BCD] focus:border-[#3A8BCD] transition"
                            required>
                    </div>
                    <div>
                    ...
                    <div>
                        <label class="flex items-center">
                            <input type="checkbox" name="is_active"
                                class="rounded border-gray-300 text-purple-600 shadow-sm focus:ring-[#3A8BCD] transition">
                            <span class="ml-2 text-[#2A5C82] font-semibold">تفعيل الاختبار</span>
                        </label>
                    </div>
                    <div id="questions">
...
<script>
    ...
    function addQuestion() {
        ...
        questionDiv.className = 'border border-[#3A8BCD]/20 rounded-xl p-4 mb-6 bg-blue-50/30 hover:shadow-lg transition';
        questionDiv.innerHTML = `
            <div class="mb-4">
                <label class="block text-sm font-bold text-[#553566] mb-1">نوع السؤال</label>
                <select name="questions[${questionCount}][question_type_id]" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:ring-[#3A8BCD] focus:border-[#3A8BCD] transition" required onchange="toggleOptionsFields(this, ${questionCount})">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="questions[${questionCount}][question]" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:ring-[#3A8BCD] focus:border-[#3A8BCD] transition" required>
            </div>
            <div class="mb-4 options-fields">
                <label class="block text-sm font-bold text-[#553566] mb-1">الخيارات</label>
                <div class="space-y-2">
                    <input type="text" name="questions[${questionCount}][options][]" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:ring-[#3A8BCD] focus:border-[#3A8BCD] transition" required>
                    <input type="text" name="questions[${questionCount}][options][]" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:ring-[#3A8BCD] focus:border-[#3A8BCD] transition" required>
                    <input type="text" name="questions[${questionCount}][options][]" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:ring-[#3A8BCD] focus:border-[#3A8BCD] transition" required>
                    <input type="text" name="questions[${questionCount}][options][]" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:ring-[#3A8BCD] focus:border-[#3A8BCD] transition" required>
                </div>
            ...
            <div class="mb-4 correct-answer-field">
                ...
                <input type="text" name="questions[${questionCount}][correct_answer]" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:ring-[#3A8BCD] focus:border-[#3A8BCD] transition" required>
            </div>
            <div class="mb-4">
                <label class="block text-sm font-bold text-[#553566] mb-1">الدرجة</label>
                <input type="number" name="questions[${questionCount}][points]" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:ring-[#3A8BCD] focus:border-[#3A8BCD] transition" required>
            </div>
            <button type="button" onclick="this.parentElement.remove()" class="text-red-600 hover:text-red-800 font-bold transition">
                حذف السؤال
            </button>
        `;
    ...
    function toggleOptionsFields(select, idx) {
        ...
        if (select.value == "1") { // اختيار من متعدد
            ...
            optionsFields.innerHTML = `
                <label class="block text-sm font-bold text-[#553566] mb-1">الخيارات</label>
                <div class="space-y-2">
                    <input type="text" name="questions[${idx}][options][]" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:ring-[#3A8BCD] focus:border-[#3A8BCD] transition" required>
                    <input type="text" name="questions[${idx}][options][]" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:ring-[#3A8BCD] focus:border-[#3A8BCD] transition" required>
                    <input type="text" name="questions[${idx}][options][]" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:ring-[#3A8BCD] focus:border-[#3A8BCD] transition" required>
                    <input type="text" name="questions[${idx}][options][]" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:ring-[#3A8BCD] focus:border-[#3A8BCD] transition" required>
                </div>
            ...
            optionsFields.style.display = '';
            correctAnswerField.querySelector('input').type = 'hidden';
            optionsFields.innerHTML = `
                <label class="block text-sm font-bold text-[#553566] mb-1">الخيارات</label>
                <div class="space-y-2">
                    <label class="flex items-center">
                        <input type="radio" name="questions[${idx}][correct_answer]" value="صح" class="mr-2 h-4 w-4 text-[#3A8BCD] border-gray-300 rounded focus:ring-[#3A8BCD] transition" required>
                        <span class="text-gray-700">صح</span>
                    </label>
                    <label class="flex items-center">
                        <input type="radio" name="questions[${idx}][correct_answer]" value="خطأ" class="mr-2 h-4 w-4 text-[#3A8BCD] border-gray-300 rounded focus:ring-[#3A8BCD] transition" required>
